...
async function isServerHealthy(): Promise<boolean> {
  try {
    const res = await fetch(`${MTPLX_URL}/v1/models`, { signal: AbortSignal.timeout(2000) });
    return res.ok;
  } catch {
    return false;
  }
...
function runPi(testLabel: string, stepNum: number, args: string, timeoutMs = 60000): boolean {
  console.log(`  [${stepNum}/4] ${testLabel}...`);
  try {
    const out = execSync(`${PI_BIN} --provider mtplx --model ${PI_MODEL} ${args}`, {
      cwd: ROOT,
      encoding: "utf8",
      timeout: timeoutMs,
      env: { ...process.env },
...
async function main() {
  ...
  labels.forEach((label, idx) => console.log(`  ${idx + 1}. ${label}: ${results[idx] ? "PASS" : "FAIL"}`));
